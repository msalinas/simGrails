...
class NavigationTagLib {
    ...
    def eachSubItem = { attrs, body ->
        ...
        // @todo The problem is that its searching on path for the base item
        // but it needs to locate it based on current controller + action if no path created
        println "EACH SI: sk $searchKey st $searchType ska $searchKeyAuto"
        ...
        println "EACH SI: root item: ${item}"
    ...
    protected doPathsIntersect(itemPath, activePath) {
        println "Checking path intersection: $itemPath vs $activePath"
    ...
    protected isPathFullyEncapsulatedBy(itemPath, activePath) {
        println "Checking encaps path: $itemPath vs $activePath"
    ...
    protected pathIsActive(itemPath, currentPath) {
        println "Checking active path: $itemPath vs $currentPath"
        def itemSize = itemPath?.size() ?: 0
    ...
    protected iterateItems = { items, var, body, linkParams, activePath ->
        ...
        items.eachWithIndex { item, i ->
            ...
            if ((isVisible == null) || (isVisible == true ) || isVisible.call()) { 
                ...
                println "Item: ${item.path} is active ${data.active} because of ${activePath}"
    ...
    def renderSubItems = { attrs ->
        ...
        o << eachSubItem(attrs, { item ->
			...
			//SE INCLUYERON SALTOS DE LINEA PARA MOSTRAR LOS SUBITEMS EN ORDEN VERTICAL
        })
        o << "</ul>"
    }
